...
class TestMdlEstimationLowerBound(unittest.TestCase):
    ...
    def test_estimate_mdl_on_simple_log_and_dfg(self):
        ...
        original_mdl = compute_mdl_score(log, dfg, verbose=True)
        print(original_mdl)
        ...
        for candidate in candidate_generator.generate_candidates(log, dfg, dfg):
            ...
            print('------------------------------')
            print(candidate)
            true_mdl = compute_mdl_score(log, candidate_dfg, verbose=True)
            print(true_mdl)
            estimated_mdl = estimate_lower_bound_mdl_score(
                    dfg, cover_on_dfg, candidate, log, dfg, verbose=True)
            print(estimated_mdl)
            self.assertLessEqual(estimated_mdl, true_mdl)
        print('=================================')
    ...
    def test_estimate_mdl_on_simple_log_and_pattern_dfg(self):
        ...
        for candidate in candidate_generator.generate_candidates(log, dfg, pattern_dfg):
            ...
            self.assertLess(estimated_mdl, true_mdl)
