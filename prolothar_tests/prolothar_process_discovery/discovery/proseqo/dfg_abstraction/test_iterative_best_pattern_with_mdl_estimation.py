...
class TestIterativeBestPatternWithMdlEstimation(unittest.TestCase):
    ...
    def test_mine_dfg_simple_example(self):
        ...
        abstracted_dfg = IterativeBestPattern(
                ...
                max_nr_of_workers=1,
                use_estimated_mdl_for_candidate_generation=True).mine_dfg(
    ...
    def test_mine_dfg_synthetic_baking_dataset(self):
        ...
        folded_dfg = IterativeBestPattern(
                CandidateGeneratorBuilder()\
                    ...
                    .with_parallels()\
                    .build(),
                use_estimated_mdl_for_candidate_generation=True
                ).mine_dfg(
                    log, PatternDfg.create_from_event_log(log), verbose=True)

        self.assertIn(folded_dfg.get_nr_of_nodes(), [12,14])
        self.assertIn(folded_dfg.get_nr_of_edges(), [21,22,24])

    def test_timebudget(self):
        print('test_timebudget started')
        log = baking.generate_log(1000, use_clustering_model=True)
        start = datetime.now()
        IterativeBestPattern(
                CandidateGeneratorBuilder()\
                    .with_choices().with_edge_removals().with_optionals()\
                    .with_sequences().with_loops()\
        ...
        self.assertLess((end-start).total_seconds(), 62)
